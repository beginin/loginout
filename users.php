...
$sql1 = "SELECT `User` FROM `ad`.`log_in_out` WHERE `Time` > DATE_ADD(NOW(), INTERVAL -60 DAY)   GROUP BY `User`";
...
foreach ($people as $k => $man)
{
    ...
    $sql = "SELECT * FROM `ad`.`log_in_out` WHERE `User`='$man_sql'   ORDER BY `ad`.`log_in_out`.`Time`  DESC limit 5 ";
    $result = mysql_query($sql, $link);
    if (!$result) die('Invalid query: ' . mysql_error());
    //echo "<br>";
    //echo $man;
